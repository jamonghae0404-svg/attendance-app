...
import { Download, ChevronLeft, ChevronRight, Users, TrendingUp, Loader2 } from 'lucide-react'
import BottomNav from '@/components/BottomNav'
import AppLogo from '@/components/AppLogo'
import { getPrograms, getRecords, getParticipants } from '@/lib/db'
...
export default function DashboardPage() {
  ...
  useEffect(() => {
    ...
    setLoading(true)
    getRecords(selectedProgram).then(records => {
  ...
  async function handleExport() {
    ...
    try {
      const [records, participants] = await Promise.all([
        getRecords(selectedProgram),
        getParticipants(selectedProgram),
      ])

      // journals는 Supabase에서 직접 가져올 수 없으므로 API에 programId를 전달
      const res = await fetch('/api/export', {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({ records, participants, programs, journals: [] }),
      })
      ...
      a.href = url
      a.download = `ALE_출석부_${new Date().toISOString().slice(0, 10)}.xlsx`
      a.click()
        ...
        {loading ? (
          ...
          <>
            ...
            <div className="bg-white rounded-xl shadow-sm border border-gray-100 p-4">
              ...
              <div className="grid grid-cols-2 gap-3">
                <div className="bg-indigo-50 rounded-xl p-4 text-center">
                  ...
                  <div className="text-3xl font-bold text-indigo-700">{weeklyStats?.actualCount ?? 0}</div>
                ...
                <div className="bg-purple-50 rounded-xl p-4 text-center">
                  ...
                  <div className="text-3xl font-bold text-purple-700">{weeklyStats?.totalCount ?? 0}</div>
            ...
            <div className="bg-white rounded-xl shadow-sm border border-gray-100 p-4">
              ...
              <div className="grid grid-cols-2 gap-3">
                <div className="bg-blue-50 rounded-xl p-4 text-center">
                  ...
                  <div className="text-3xl font-bold text-blue-700">{monthlyStats?.actualCount ?? 0}</div>
                ...
                <div className="bg-cyan-50 rounded-xl p-4 text-center">
                  ...
                  <div className="text-3xl font-bold text-cyan-700">{monthlyStats?.totalCount ?? 0}</div>
            ...
            <div className="bg-gray-50 rounded-xl p-4 text-sm text-gray-500 space-y-1.5">
              ...
              <p>• 엑셀 내보내기 시 출석 로그, 이용자 요약이 포함됩니다</p>
            </div>
          </>
        )}
      </div>
